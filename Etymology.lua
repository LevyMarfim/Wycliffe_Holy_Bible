...
heuene = [[\textbf{Heuene} - heaven \phnm{/ˈhɛv(ə)n/ (n.)}: From OE \textit{heofon} ``home of God,'' earlier ``the visible sky, firmament,'' probably from Proto-Germanic *\textit{hibin}-, a dissimilation of *\textit{himin}- (source also of Low German \textit{heben}, ON \textit{himinn}, Gothic \textit{himins}, OF \textit{himul}, Dutch \textit{hemel}, German \textit{Himmel} ``heaven, sky''). The abode of God, heaven, paradise.]]

--
--	I
--

idel = [[\textbf{Idel} - idle \phnm{/ˈʌɪd(ə)l/ (adj.)}: OE \textit{idel} ``empty, void; vain; worthless, useless,'' from Proto-West Germanic *\textit{idla}- (source also of OS \textit{idal}, OF \textit{idel} ``empty, worthless,'' Old Dutch \textit{idil}, OHG \textit{ital}, German \textit{eitel} ``vain, useless, mere, pure''), a word of unknown origin.]]
...
theArticle = [[\textbf{Þe} - the \phnm{/ðə/, /ðɪ/, /ðiː/} (def. art.): Late OE \textit{þe}, nominative masculine form of the demonstrative pronoun and adjective. After c.950, it replaced earlier \textit{se} (masc.), \textit{seo} (fem.), \textit{þæt} (neuter), and probably represents \textit{se} altered by the \textit{th}- form which was used in all the masculine oblique cases. OE \textit{se} is from PIE root *\textit{so}- ``this, that'' (source also of Sanskrit \textit{sa}, Avestan \textit{ha}, Greek \textit{ho}, \textit{he} ``the,'' Irish and Gaelic \textit{so} ``this''). The \textit{þ}- forms, see \textit{that}. The \textit{s}- forms were entirely superseded in English by mid-13c., excepting a slightly longer dialectal survival in Kent. OE used 10 different words for ``the,'' but did not distinguish ``the'' from ``that.'' \textit{That} survived for a time as a definite article before vowels (\textit{that one} or \textit{that other}).]]

--
--	V
--

voide = [[\textbf{Voide} - void \phnm{/vɔɪd/ (adj.)}: c. 1300, ``unoccupied, vacant,'' from Anglo-French and Old French \textit{voide}, \textit{viude} ``empty, vast, wide, hollow, waste, uncultivated, fallow,'' as a noun, ``opening, hole; loss,'' from Latin \textit{vocivos} ``unoccupied, vacant,'' related to \textit{vacare} ``be empty,'' from PIE *\textit{wak}-, extended form of root *\textit{eue}- ``to leave, abandon, give out.'' Meaning ``lacking or wanting'' (something) is recorded from early 15c. Meaning ``legally invalid, without legal efficacy'' is attested from mid-15c.]]
